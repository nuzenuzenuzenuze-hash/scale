...
import { deleteTrend } from "@/lib/actions/trends";

export function DeleteTrendButton({ trendId }: { trendId: string }) {
  const [isPending, startTransition] = useTransition();

  return (
    <button
      onClick={() => {
        if (confirm("¿Eliminar esta señal de tendencia?")) {
          startTransition(() => deleteTrend(trendId));
        }
      }}
      disabled={isPending}
